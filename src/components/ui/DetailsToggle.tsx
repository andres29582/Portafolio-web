...
export function DetailsToggle({ children, label, className = "" }: DetailsToggleProps) {
  return (
    <details className={`group ${className}`}>
      <summary className="focus-ring inline-flex min-h-11 cursor-pointer list-none items-center rounded-md border border-line bg-white px-4 py-2 text-sm font-bold text-ink transition hover:border-signal hover:text-signal">
        <span>{label}</span>
